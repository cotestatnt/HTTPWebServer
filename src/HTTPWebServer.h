/*
  HTTPWebServer.h - Dead simple web-server.
...
extern const String HTTP_EMPTY_STRING;
class HTTPWebServer;
...
class HTTPWebServer {
public:
  explicit HTTPWebServer(int port = 80);
  virtual ~HTTPWebServer();
  ...
  typedef std::function<bool(HTTPWebServer &server)> FilterFunction;
  RequestHandler &on(const HTTPUri &uri, THandlerFunction fn);
  RequestHandler &on(const HTTPUri &uri, HTTPMethod method, THandlerFunction fn);
  RequestHandler &on(const HTTPUri &uri, HTTPMethod method, THandlerFunction fn, THandlerFunction ufn);  //ufn handles file uploads
  ...
  HTTPWebServer &addMiddleware(Middleware *middleware);
  HTTPWebServer &addMiddleware(Middleware::Function fn);
  HTTPWebServer &removeMiddleware(Middleware *middleware);
