...
public:
    ...
    void onConnected(std::function<void(const char*, const char*)> callback) { _connectedCallback = callback; }

    // Callback che verrà chiamato quando la configurazione è cambiata
    void onConfigChanged(std::function<void(void)> callback) { _configChangedCallback = callback; }

    // Metodi per ottenere informazioni sulla connessione
    String getCurrentSSID() const { return WiFi.SSID(); }
    IPAddress getLocalIP() const { return WiFi.localIP(); }
    IPAddress getGateway() const { return WiFi.gatewayIP(); }
    IPAddress getSubnet() const { return WiFi.subnetMask(); }
